  ...
  class SeleniumChromeBuilder
    ...
    def build
      ...
      Capybara.register_driver :selenium_chrome do |app|
        ...
        if proxy = @config[:proxy].presence
          ...
          if %w(http socks5).include?(type)
            if user.nil? && password.nil?
              ...
              begin
                real_proxy = "http://#{user}:#{password}@#{ip}:#{port}"
              ...
              # profile.proxy << proxy_object
              # p driver_options.args
        ...
        if @proxy_object
          caps = Selenium::WebDriver::Remote::Capabilities.chrome(:proxy => @proxy_object)
